...
**확인하지 못한 것**: **화면을 사람 눈으로 보지 못했다.** Windows 데스크톱
빌드까지는 됐지만(`flutter build windows --debug` 성공, 앱 실행됨) 개발 빌드는
시작 메뉴에 없어 자동화 도구가 창을 잡지 못한다. 보드 렌더 · 카드 메뉴 이동 ·
밖에서 만든 이슈의 실시간 반영은 **직접 띄워 확인해야 한다.**
