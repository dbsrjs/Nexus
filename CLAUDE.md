...
**아직 없는 것**: 멘션 · 핀 · DM, 첨부, 이슈, 저장소 연동, AI.
...
| **7-4** ← 다음 | **멘션** — `Mention` 모델은 있다. 파싱 · 알림 팬아웃 · 안 읽은 멘션 뱃지 | |
| 7-5~ | 핀 → 첨부 → 이슈 · 스프린트 → repos(웹훅 · 열람 프록시) → PR → 인덱싱 → AI | |
...
- **앱 테스트에 위젯 · 통합 테스트가 없다.** `app/test/` 에 50개 — `Env` 2 · 반응형 경계 4 · 채널 묶기 6 · 전송 큐 14 · 리액션 10 · 스레드 7 · **답장 7**. 인메모리 drift 로 실제 DB 동작까지 덮지만, **화면 계층은 여전히 실기기 확인에만 의존한다.**
