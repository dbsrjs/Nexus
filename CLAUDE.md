...
| **상태** | **1~12단계는 `main` 에, 13-1 은 끝나 `feat/13-ai` 에 있다(아직 `main` 으로 합치지 않았다)** — 오프라인 대화 · 파일 · 이슈 보드 · GitHub 연동(웹훅 · 열람 · PR) · 저장소 인덱싱 · 대화 요약(AI). 12단계와 13-1 은 각각 진짜 GitHub · 진짜 임베딩, 진짜 Gemini 로 완주했다. **다음은 13-2(이슈 초안)**. 단계 표는 §5, 단계마다의 경과는 [docs/진행-기록.md](docs/진행-기록.md). **새 PC 셋업은 [docs/11-인수인계.md](docs/11-인수인계.md) §2** — `.env` 에서 손으로 채울 값 셋과 검증 기대값(나머지는 `npm run env:setup` 이 만든다) |
...
ai/           대화 요약(13-1) — ai_runs 큐 · promptHash 캐시 · 러너 · 소켓 알림
...
**1~12단계와 13-1 이 끝났다** — 단계 표는 §5. 단계마다 무엇을 왜 그렇게 정했는지, 무엇을
...
채널별 권한 · 이슈 초안(13-2) · 코드 질의(13-3). `notifications` · `permissions`
