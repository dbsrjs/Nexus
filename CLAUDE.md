...
**아직 없는 것**: 스레드 · 멘션 · 핀 · DM, 첨부, 이슈, 저장소 연동, AI.
...
| **7-2** ← 다음 | **스레드** — `parentId` 는 이미 있다. 답글 목록 API + 화면(새 라우트)이 남았다 | |
| 7-3~ | 멘션 → 핀 → 첨부 → 이슈 · 스프린트 → repos(웹훅 · 열람 프록시) → PR → 인덱싱 → AI | |
...
- **앱 테스트에 위젯 · 통합 테스트가 없다.** `app/test/` 에 36개 — `Env` 2 · 반응형 경계 4 · 채널 묶기 6 · 전송 큐 14 · **리액션 10**. 인메모리 drift 로 실제 DB 동작까지 덮지만, **화면 계층은 여전히 실기기 확인에만 의존한다.**
