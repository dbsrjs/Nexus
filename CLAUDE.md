...
- **`repo_index_chunks` 에 어떤 모델로 만든 벡터인지가 없다.** 컬럼은 `path` · `lang` · `embedding` · `commit_sha` 뿐이다. 그래서 **`EMBEDDING_PROVIDER` 나 `EMBEDDING_MODEL` 을 바꾼 뒤 그냥 push 하면** 웹훅이 `indexedCommitSha` 를 기준으로 **증분**을 돌려 바뀐 파일만 새 모델 벡터가 되고 나머지는 옛 모델로 남는다. 섞인 인덱스는 **오류를 내지 않고 순위만 틀린다** — 설계 §6 이 "가장 나쁜 실패"라고 적은 그것이다. 지금은 **사람이 기억해서 전체 재인덱싱을 눌러야 한다.** 막으려면 모델 이름을 `repos` 나 청크에 기록해 두고 다르면 전체로 떨어뜨리면 되는데, 2026-09-10 에는 provider 를 고르는 것이 먼저라 미뤘다.
- **컨트롤러 · 서비스의 실 DB 검증은 계약 검증 스크립트가 담당한다.** 서버 단위 테스트 251개는 순수 로직 · 가드 · 권한 규칙만 덮는다. 이 경계는 의도한 것이다 — 단위 테스트로 DB 동작을 증명하려 하면 §6 의 실수를 반복한다. **계약 검증은 2026-08-21 부터 CI 에서 push 마다 돈다 — 14종 508 케이스**(여기에 DB 없이 도는 정적 검사 둘 — `check:migrations` · `check:sql-time` — 이 따로 붙는다)(12단계에서 `check:indexing` 34개가 더해져 13종 474 에서 늘었다. 그 전엔 11단계에서 `check:pulls` 35개가 더해져 12종 439 에서 늘었다)(`서버 통합` 잡, 계약 검증 단계 19초). 그 전까지는 사람이 기억해 타이핑할 때만 돌았다. 헬퍼 중복도 2026-08-21 에 걷었다 — `api()` 11벌 · `signup()` 10벌 등이 `server/scripts/lib/` 넷으로 모였다(스크립트에서 473줄이 지워졌다). **남은 빚은 러너가 아니라 단언 규율이다** — 10-2b 의 `undefined === undefined` 는 프레임워크로 바꿔도 통과한다(`assert.strictEqual(undefined, undefined)`). 그 케이스는 손으로 고쳐 뒀고, 같은 종류가 다시 나오면 그때 장치를 만든다.
