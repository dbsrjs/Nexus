...
| **상태** | **1~12단계와 13-1 이 main 에 있다** — 오프라인 대화 · 파일 · 이슈 보드 · GitHub 연동(웹훅 · 열람 · PR) · 저장소 인덱싱 · 대화 요약(AI). 12단계와 13-1 은 각각 진짜 GitHub · 진짜 임베딩, 진짜 Gemini 로 완주했다. **다음은 13-2(이슈 초안)**. 단계 표는 §5, 단계마다의 경과는 [docs/진행-기록.md](docs/진행-기록.md). **새 PC 셋업은 [docs/11-인수인계.md](docs/11-인수인계.md) §2** — `.env` 에서 손으로 채울 값 셋과 검증 기대값(나머지는 `npm run env:setup` 이 만든다) |
...
| `npm run check:indexing` | 저장소 인덱싱 계약 검증(40개) — 연결 시 적재 · 거르기(바이너리 · 대용량 · 생성 파일) · 벡터 검색 순위 · 증분 재인덱싱(push 웹훅 → compare, 이름 변경(renamed) 갈래 포함) · force-push(compare 404 로 실제로 응답한 횟수까지 확인) · **임베딩 모델 변경 시 compare 없이 전체**(기록된 모델을 DB 에서 직접 바꿔 흉내 낸다 — 계약 검증에서 DB 를 만지는 유일한 곳) · 기능 브랜치 무시(12단계). **가짜 GitHub(4599)을 스스로 띄운다** — `check:browse` 와 같은 `.env` 에 **`EMBEDDING_PROVIDER=fake` 가 더 필요하다** |
...
| `npm run check:ai` | AI(대화 요약) 계약 검증(설정된 서버에서 32개) — LLM 캐시 · 큐 · 소켓 알림 · 멘션 치환 실증(13-1). **`LLM_PROVIDER=gemini` 로 뜬 서버에서는 무료 티어 쿼터를 쓴다** — `fake` 로 돌리는 쪽이 기본이다. 미설정 503 분기는 `check:oauth` 와 같은 패턴으로 `LLM_PROVIDER` 를 비운 채 한 번 더 돌려야 확인된다(스크립트가 안내를 찍는다) |
...
**1~12단계가 끝났다** — 단계 표는 §5. 단계마다 무엇을 왜 그렇게 정했는지, 무엇을
...
**아직 없는 것**: AI · DM · 프레즌스 · 타이핑 표시 · 알림(인앱 · 멘션 알림 · 푸시) ·
채널별 권한. `ai` · `notifications` · `permissions` 모듈은 미이관이다(§3 끝).
실시간 서버에서 아직 쓰지 않는 이벤트는 `typing` · `presence:changed` 다.
...
- **모델을 바꾼 뒤 다음 인덱싱 전까지는 검색이 옛 벡터를 새 모델의 질의로 찾는다.** 2026-09-17 에 `repos.indexedEmbeddingModel` 을 기록해 **인덱싱은** 모델이 다르면 전체로 떨어지게 했지만(진행 기록 «빚 정리»), `search()` 는 기록을 보지 않는다. 오류 없이 순위만 틀리는 구간이 push · 다시 태우기 전까지 남는다. 막으려면 `search()` 가 기록과 현재 모델을 비교해 거절하면 되는데, 전체 재인덱싱 중(기록이 `null`)에 검색을 어떻게 할지 함께 정해야 해 13단계에서 AI 가 검색을 부르는 모양을 보고 정한다.
- **컨트롤러 · 서비스의 실 DB 검증은 계약 검증 스크립트가 담당한다.** 서버 단위 테스트 315개는 순수 로직 · 가드 · 권한 규칙만 덮는다. 이 경계는 의도한 것이다 — 단위 테스트로 DB 동작을 증명하려 하면 §6 의 실수를 반복한다. **계약 검증은 2026-08-21 부터 CI 에서 push 마다 돈다 — 15종 554 케이스**(여기에 DB 없이 도는 정적 검사 둘 — `check:migrations` · `check:sql-time` — 이 따로 붙는다)(13-1 에서 `check:ai` 32개가 더해져 15종 554 로 늘었다. 2026-09-17 빚 정리에서 `check:indexing` 6개 · `check:realtime` 8개가 더해져 508 에서 늘었다. 12단계에서 `check:indexing` 34개가 더해져 13종 474 에서 늘었다. 그 전엔 11단계에서 `check:pulls` 35개가 더해져 12종 439 에서 늘었다)(`서버 통합` 잡, 계약 검증 단계 19초). 그 전까지는 사람이 기억해 타이핑할 때만 돌았다. 헬퍼 중복도 2026-08-21 에 걷었다 — `api()` 11벌 · `signup()` 10벌 등이 `server/scripts/lib/` 넷으로 모였다(스크립트에서 473줄이 지워졌다). **남은 빚은 러너가 아니라 단언 규율이다** — 10-2b 의 `undefined === undefined` 는 프레임워크로 바꿔도 통과한다(`assert.strictEqual(undefined, undefined)`). 그 케이스는 손으로 고쳐 뒀고, 같은 종류가 다시 나오면 그때 장치를 만든다.
- **AI 큐의 실패 갈래와 「모델이 바뀌면 캐시가 적중하지 않음」이 계약 검증에 없다(13-1).** 리스 만료 복구 · 5xx 3회 소진 · fatal 즉시 포기를 `check:ai` 로 재현하려면 실패를 주입할 수 있는 fake LLM 어댑터가 필요한데, 지금 `fake` 는 항상 즉시 성공만 한다 — 인덱싱 큐의 같은 자리(§4 «12 실제 태우기» 이후에도 남은 빚)와 같은 모양이다. 단위 테스트(`classifyFailure` · `shouldGiveUp`)가 대신 덮는다.
