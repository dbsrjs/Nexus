...
**아직 없는 것**: 스레드 · 리액션 · 멘션 · 핀 · DM, 첨부, 이슈, 저장소 연동, AI.
...
| **7~** ← 다음 | **기능 단위로 서버 + 앱을 함께**: 스레드 · 리액션 · 멘션 · 핀 → 첨부 → 이슈 · 스프린트 → repos(웹훅 · 열람 프록시) → PR → 인덱싱 → AI | |
...
- **앱 테스트에 위젯 · 통합 테스트가 없다.** `app/test/` 에 26개 — `Env` 2 · 반응형 경계 4 · 채널 묶기 6 · **전송 큐 14**. 큐 테스트는 인메모리 drift 로 실제 DB 동작까지 덮지만(순서 · 재시도 · 캐시 재생성 생존), **화면 계층은 여전히 실기기 확인에만 의존한다.**
