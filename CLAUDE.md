...
| **상태** | **1~12단계가 main 에 있다** — 오프라인 대화 · 파일 · 이슈 보드 · GitHub 연동(웹훅 · 열람 · PR) · 저장소 인덱싱. 12단계는 진짜 GitHub · 진짜 임베딩으로도 완주했다. **다음은 13단계(AI)**. 단계 표는 §5, 단계마다의 경과는 [docs/진행-기록.md](docs/진행-기록.md). **새 PC 셋업은 [docs/11-인수인계.md](docs/11-인수인계.md) §2** — `.env` 에서 손으로 채울 값 셋과 검증 기대값(나머지는 `npm run env:setup` 이 만든다) |
