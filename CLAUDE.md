...
| **상태** | 사내 메신저 → 개인 프로젝트로의 **전환 완료**. **10-1(저장소 웹훅 수신)까지 main 에 들어갔다** — 오프라인으로 켜도 대화가 보이고(Phase 0), 파일을 주고받고, 칸반 보드에 이슈가 뜬다. **10-2(GitHub 연동)와 10-3a(저장소 열람)가 main 에 들어갔고, 10-3b(커밋 열람)는 `feat/commit-browse` 에서 끝났다** — 앱에서 저장소를 골라 붙이면 push 가 채널로 들어오고, 그 한 줄을 누르면 들어온 커밋과 바뀐 파일이 보이며, 파일을 누르면 그 시점의 전문이 열린다. 전부 진짜 GitHub 으로 확인했다. **다음은 11단계(PR)**. **다른 PC 에서 이어받으면 [docs/10-2a-인수인계.md](docs/10-2a-인수인계.md) 의 `.env` 절을 먼저 읽을 것 — `PUBLIC_BASE_URL` 이 하나 더 필요하다** |
...
- **앱 테스트에 통합 테스트가 없다.** 위젯 테스트는 9-3 에서 처음 붙었지만 계획 줄(스프린트 · 포인트) 하나뿐이다. `app/test/` 에 106개 — `Env` 2 · 반응형 경계 4 · 채널 묶기 6 · 전송 큐 14 · 리액션 10 · 스레드 7 · 답장 7 · **멘션 34** · 핀 7 · **첨부 15**. 인메모리 drift 로 실제 DB 동작까지 덮지만, **화면 계층은 여전히 실기기 확인에만 의존한다.** 멘션 입력창의 커스텀 `TextEditingController`(커서 · IME)도 실기기 확인에만 기댄다.
