...
**확인한 것**: `npm run check:mentions` 18개(실서버·실DB) · 서버 62개 · 앱 71개.
에뮬레이터로 `@` 입력 → 후보 표시(자기 자신 제외) → 선택 시 id 삽입 → 전송 →
`@이름` 강조 렌더까지 확인했고, **밖에서 나를 멘션하니 채널 목록에 멘션 뱃지가
따로 떴다.**
...
- **앱 테스트에 위젯 · 통합 테스트가 없다.** `app/test/` 에 78개 — `Env` 2 · 반응형 경계 4 · 채널 묶기 6 · 전송 큐 14 · 리액션 10 · 스레드 7 · 답장 7 · 멘션 21 · **핀 7**. 인메모리 drift 로 실제 DB 동작까지 덮지만, **화면 계층은 여전히 실기기 확인에만 의존한다.**
