...
| **상태** | 사내 메신저 → 개인 프로젝트로의 **전환 완료**. **9-1(이슈 보드)까지 끝났다** — 오프라인으로 켜도 대화가 보이고(Phase 0), 파일을 주고받고, 칸반 보드에 이슈가 뜬다. 다음은 9-2(이슈 상세 · 댓글 · 라벨 · 대화→이슈) |
...
| `npm run check:issues` | 이슈 계약 검증(47개). 자체 계정을 쓴다. **컬럼 상한(200)과 재채번까지 태우므로 다른 스크립트보다 오래 걸린다** |
...
**아직 없는 것**: DM, 라벨 · 대화 → 이슈(9-2b), 스프린트 · 번다운(9-3),
