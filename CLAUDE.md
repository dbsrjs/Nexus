...
| **상태** | 사내 메신저 → 개인 프로젝트로의 **전환 완료**. **10-1(저장소 웹훅 수신)까지 main 에 들어갔다** — 오프라인으로 켜도 대화가 보이고(Phase 0), 파일을 주고받고, 칸반 보드에 이슈가 뜬다. **지금 작업 중인 것은 `feat/github-oauth` 의 10-2a(GitHub 계정 연결)** — 서버는 끝났고 앱이 남았다. **다른 PC 에서 이어받으면 [docs/10-2a-인수인계.md](docs/10-2a-인수인계.md) 를 먼저 읽을 것** |
...
### 10-2a 진행 중 (GitHub 계정 연결) — 서버 완료, 앱이 남았다
...
- **검증은 가짜 GitHub 을 띄워 한다**(`GITHUB_API_BASE` · `GITHUB_OAUTH_BASE`).
  ...
  `check:oauth` 가 진짜 GitHub 없이 돈다

**확인한 것**: `npm run check:oauth` **38개** · 서버 **138개** · 앱 **129개** ·
`flutter analyze` 0건. **미설정 503 분기**도 키를 비우고 재시작해 직접 확인했다
(서버는 정상 부팅했고 `start()` 만 503).

**확인하지 못한 것**: 진짜 GitHub 으로의 종단 확인(OAuth App 등록이 필요하고
사람만 할 수 있다) · 앱 화면(Task 8·9 가 아직 없다).
...
| 10-2a | **GitHub 계정 연결** — OAuth 시작 · 콜백 · 토큰 암호화 · `oauth:connected` · 앱 연결 화면 | 🔨 **서버 완료 · 앱 진행 중**(`feat/github-oauth`) |
