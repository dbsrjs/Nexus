...
| **상태** | 사내 메신저 → 개인 프로젝트로 **전환 중**. 서버 4단계(실시간 최소) 완료. **앱은 5단계 완료 + 6-1(drift 캐시 읽기 경로) — 오프라인으로 켜도 대화가 보인다** |
...
data/local/app_database.dart  drift 캐시 — 스페이스 · 채널 · 카테고리 · 메시지
...
**아직 없는 것**: 오프라인 전송 큐(6-2), 스레드 · 리액션 · 멘션 · 핀 · DM, 첨부,
이슈, 저장소 연동, AI.
...
| **6-2** ← 다음 | **전송 큐** — 오프라인에서 쓴 메시지를 재연결 시 흘려보낸다 → Phase 0 달성(실사용 가능) | |
...
- **앱 테스트가 아직 얇다.** `app/test/` 에 12개 — `Env` 2 · 반응형 경계 4 · 채널 묶기 6. 순수 로직 위주이고, **위젯 · 통합 테스트는 아직 없다.** 낙관적 전송과 오프라인 읽기는 실기기로만 확인했다.
