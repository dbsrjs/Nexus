...
- **시각은 텍스트(ISO-8601)로 저장한다** — `DriftDatabaseOptions(storeDateTimeAsText: true)`. drift 기본값은 Unix **초**라 밀리초가 잘리고, 같은 초에 도착한 메시지끼리 정렬이 동률이 되어 순서가 뒤집힌다. 실제로 겪었다.
...
자동 전송 → 순서 유지. `flutter analyze` 0건, `flutter test` 25/25.

검증 중 **순서가 뒤집히는 버그**를 잡았다. 서버 기록은 `.192` · `.397` 로 멀쩡한데
화면만 뒤집혔다 — drift 가 시각을 **초 단위**로 저장해 둘이 동률이 됐기 때문이다.
저장 형식을 텍스트로 바꿔 밀리초를 보존한다. 재현 조건이 "같은 초에 두 개"라
테스트로 고정해 뒀다.
...
- **앱 테스트에 위젯 · 통합 테스트가 없다.** `app/test/` 에 25개 — `Env` 2 · 반응형 경계 4 · 채널 묶기 6 · **전송 큐 13**. 큐 테스트는 인메모리 drift 로 실제 DB 동작까지 덮지만(순서 · 재시도 · 캐시 재생성 생존), **화면 계층은 여전히 실기기 확인에만 의존한다.**
- **전송 실패 UI(재시도 · 삭제)를 실기기로 태우지 못했다.** 규칙은 단위 테스트로 덮었지만(401 · 404 즉시 실패, 서버 오류 3회, 재시도), 화면에서 실제로 그 상태를 만들려면 채널을 지우거나 권한을 뺏어야 해 재현하지 않았다.
- **drift 웹 자산을 아직 붙이지 않았다.** `sqlite3.wasm` · `drift_worker.js` 가 `app/web/` 에 없어 **웹에서 캐시를 여는 순간 런타임에서 실패한다.** Android · Windows 는 영향이 없다. 웹을 확인하는 슬라이스 경계에서 갚는다.
