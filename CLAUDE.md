...
| **상태** | 사내 메신저 → 개인 프로젝트로의 **전환 완료**. **10-1(저장소 웹훅 수신)까지 main 에 들어갔다** — 오프라인으로 켜도 대화가 보이고(Phase 0), 파일을 주고받고, 칸반 보드에 이슈가 뜬다. **10-2(GitHub 연동) 전체가 `feat/github-oauth` 에서 끝났다** — 계정 연결(10-2a)과 저장소 자동 등록(10-2b) 모두 진짜 GitHub 으로 확인했다. 앱에서 저장소를 골라 붙이면 push 가 채널로 들어온다. 다음은 10-3(열람 프록시). **다른 PC 에서 이어받으면 [docs/10-2a-인수인계.md](docs/10-2a-인수인계.md) 의 `.env` 절을 먼저 읽을 것 — `PUBLIC_BASE_URL` 이 하나 더 필요하다** |
