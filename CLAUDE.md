...
| **7-3** ← 다음 | **멘션** — `Mention` 모델은 있다. 파싱 · 알림 팬아웃 · 안 읽은 멘션 뱃지 | |
| 7-4~ | 핀 → 첨부 → 이슈 · 스프린트 → repos(웹훅 · 열람 프록시) → PR → 인덱싱 → AI | |
...
- **앱 테스트에 위젯 · 통합 테스트가 없다.** `app/test/` 에 43개 — `Env` 2 · 반응형 경계 4 · 채널 묶기 6 · 전송 큐 14 · 리액션 10 · **스레드 7**. 인메모리 drift 로 실제 DB 동작까지 덮지만, **화면 계층은 여전히 실기기 확인에만 의존한다.**
- **`prisma migrate dev` 가 만든 SQL 을 그대로 믿으면 안 된다.** pgvector HNSW 인덱스처럼 Prisma 가 표현하지 못해 수동 관리하는 것을 드리프트로 보고 `DROP INDEX` 를 끼워 넣는다. 7-1 에서 실제로 겪었다 — 마이그레이션 파일은 커밋 전에 읽을 것.
