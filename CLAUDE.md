...
| **상태** | 사내 메신저 → 개인 프로젝트로의 **전환 완료**. **10-1(저장소 웹훅 수신)까지 main 에 들어갔다** — 오프라인으로 켜도 대화가 보이고(Phase 0), 파일을 주고받고, 칸반 보드에 이슈가 뜬다. **10-2(GitHub 연동)가 main 에 들어갔고, 10-3a(저장소 열람)는 `feat/repo-browse` 에서 끝났다** — 앱에서 저장소를 골라 붙이면 push 가 채널로 들어오고, 그 저장소 안의 코드를 브랜치·폴더를 타고 열어 볼 수 있다. 둘 다 진짜 GitHub 으로 확인했다. 다음은 10-3b(커밋 열람). **다른 PC 에서 이어받으면 [docs/10-2a-인수인계.md](docs/10-2a-인수인계.md) 의 `.env` 절을 먼저 읽을 것 — `PUBLIC_BASE_URL` 이 하나 더 필요하다** |
...
| `npm run check:browse` | 저장소 열람 계약 검증(33개). **가짜 GitHub(4599)을 스스로 띄운다** — `check:oauth` 와 같은 `.env` 를 쓴다. 연결 · 등록이 주제인 그쪽과 섞지 않았다 |
...
| 10-3b | **커밋 열람** — `commits` · 커밋 상세 · 채널 메시지에서 진입 | |
| 11~ | PR → 인덱싱 → AI | |
