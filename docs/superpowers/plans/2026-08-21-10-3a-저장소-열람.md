...
async function main() {
  ...
  const huge = await api('GET', at('blob', '?path=huge.bin'), { token: alice.token });
  check('GitHub 이 본문을 안 주면 unavailable', huge.json?.omitted === 'unavailable', JSON.stringify(huge.json));
