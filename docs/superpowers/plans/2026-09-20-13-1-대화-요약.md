...
- Produces:
  - `sealed class AiSummaryState` — `AiIdle` · `AiRunning(runId)` · `AiReady(AiRun)` · `AiFailed(ApiFailure)`
  - `String aiMessageFor(ApiFailure failure)` — AI 화면 전용 문구
...
> **새 실패 enum 을 만들지 않는다.** `app/lib/data/api/api_failure.dart` 의
> `ApiFailure`(`unauthorized` · `notFound` · `network` · `tooLarge` ·
> `badRequest` · `server`)가 이미 같은 일을 하고, `AiApi` 는 `DioException` 을
> `ApiException(classifyDioException(e))` 로 감싸 던진다(Task 11 이 저장소
> 관례를 따라 그렇게 했다). 따라서 **`on DioException` 이 아니라
> `on ApiException` 으로 잡고**, `err.failure` 를 그대로 쓴다.
>
> 문구만 AI 화면 것이 필요하다 — `messageFor()` 는 「찾을 수 없습니다」처럼
> 일반적이라 요약 맥락에 맞지 않는다. `aiMessageFor()` 를 따로 둔다.
