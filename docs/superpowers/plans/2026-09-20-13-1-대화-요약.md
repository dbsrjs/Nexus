...
export class AiService {
  ...
  private async loadMessages(
    ...
    // **상한은 정규화 전 원본 길이로 본다.** 중복을 접은 뒤에 재면 같은 id 를
    // 201번 보내 상한을 우회하는 통로가 생긴다.
    if (dto.messageIds.length > MAX_TRANSCRIPT_MESSAGES) {
      throw new BadRequestException(
        `한 번에 ${MAX_TRANSCRIPT_MESSAGES}개까지 요약할 수 있습니다`,
      );
    }
    // **중복을 접는다.** SQL `IN` 이 중복을 접으므로 아래 개수 비교가
    // 「보이는데도 404」로 오분류한다. 판단 #4 의 취지는 누락 방지이지
    // 중복 거부가 아니다.
   ...
   *
   * **`spaceMember` 를 거친다 — 전역 `User` 를 조회하면 테넌트가 샌다.**
   * 본문의 `<@id>` 는 사용자가 친 텍스트라 검증되지 않는다. `MentionsService`
   * 가 「멤버 아닌 id 는 버린다」고 하는 것은 **`Mention` 행 생성**에만
   * 적용되고 본문 문자열은 그대로 남는다(판단 #4). 그래서 다른 스페이스
   * 사용자의 UUID 를 본문에 써 넣고 요약하면 무관한 사람의 실명이 프롬프트와
   * 요약문에 실린다. `mentions.service.ts` 가 같은 형태로 가둔다.
   *
   * 스페이스 밖 id 는 Map 에 없으므로 `withNames()` 의 `@(알 수 없음)` 폴백이
   * 처리한다 — 새 분기를 만들지 않는다.
   */
