 ...
 *
 * **4,000 에서 3,000 으로 내렸다 (2026-09-10).** 2,048 토큰 모델에 한국어가
 * 섞인 4,000자를 넣으면 **잘리기 직전**이라는 것을 실측했다(꼬리를 붙여도
 * 벡터가 거의 안 움직였다 — 유사도 0.99944). 한국어는 토큰당 글자 수가 적어
 * 같은 글자 수라도 영문보다 토큰을 많이 쓴다.
 *
 * **넘치면 오류가 아니라 조용한 잘림**이라 눈치챌 방법이 없다 — 그래서 여유를
 * 둔다. 이 저장소의 청크 평균이 1,600자 안팎이라 실제로 걸리는 것은 긴 줄이
 * 몰린 파일뿐이다.
 */
