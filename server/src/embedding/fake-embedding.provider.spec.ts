...
describe('FakeEmbeddingProvider', () => {
  ...
  it('차원은 언제나 768 이다 — 스키마와 HNSW 인덱스가 그 값이다', async () => {
    const [v] = await provider.embed(['안녕'], 'document');
  ...
  it('결정적이다 — 같은 글은 언제나 같은 벡터다', async () => {
    const [a] = await provider.embed(['socket reconnect handler'], 'document');
    const [b] = await provider.embed(['socket reconnect handler'], 'document');
    expect(a).toEqual(b);
  });

  it('단위 벡터다', async () => {
    const [v] = await provider.embed(['단위 길이 확인'], 'document');
  ...
  it('빈 글도 768 차원을 준다 — 삽입이 터지면 안 된다', async () => {
    const [v] = await provider.embed([''], 'document');
    expect(v).toHaveLength(EMBEDDING_DIMENSIONS);
  });
...
// **task 를 무시하는 것이 요구 조건이다.** 진짜 provider 는 문서와 질의를
  ...
  it('document 와 query 가 같은 벡터를 준다 — 대칭이어야 한다', async () => {
    const [asDoc] = await provider.embed(['소켓 재연결 토큰'], 'document');
    const [asQuery] = await provider.embed(['소켓 재연결 토큰'], 'query');
